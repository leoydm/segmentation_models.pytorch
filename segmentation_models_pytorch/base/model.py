...
class DiffDDModel(SegmentationModel):
    ...
    def forward(self, first, second):
        ...
        first_features = self.encoder(first)
        first_features = [first_features[0]] + first_features[-self.decoder_depth:]
        second_features = self.encoder(second)
        second_features = [second_features[0]] + second_features[-self.decoder_depth:]
        combined_features = [torch.cat((first_features[i], second_features[i]), dim=1) for i in range(len(first_features))]
        # [print(f.shape) for f in combined_features]
        decoder_output = self.decoder(combined_features)
...
class RefinerModel(SegmentationModel):
    ...
    def forward(self, first, second):
        ...
        combined_features = [torch.cat((first_features[i], second_features[i]), dim=1) for i in range(len(first_features))]
        decoder_output = self.decoder(combined_features)
        mid_masks = self.segmentation_head(decoder_output)
        # masks_ft = self.mask_feature_head(decoder_output)
        mask_features = self.mid_mask_encoder(mid_masks)
        # print([ft.size() for ft in first_features])
        # print([ft.size() for ft in second_features])
        # print([ft.size() for ft in mask_features])
...
class RRefinerModel(SegmentationModel):
